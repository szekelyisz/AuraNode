...
public:
    ...
    void loge(const char *message) { log(Severity::ERR, message); }
    void logw(const char *message) { log(Severity::WARN, message); }

    void logfe(const char* format, ...) __attribute__((format(printf, 2, 3)));
    void logfw(const char* format, ...) __attribute__((format(printf, 2, 3)));
...
private:
    ...
    void logv(Severity prio, const char *format, va_list args);
