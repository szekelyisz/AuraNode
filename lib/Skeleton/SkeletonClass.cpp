...
bool SkeletonClass::init(SkeletonConfig* c, const char* appname) {
    ...
    if(!SPIFFS.begin()) {
        logger.log(SerialSysLog::CRIT, "Unable to mount filesystem");
        return false;
    } else {
        logger.log(SerialSysLog::INFO, "Filesystem mounted");
    }
