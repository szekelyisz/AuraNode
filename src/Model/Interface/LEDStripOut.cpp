...
LEDStripOut::LEDStripOut(::AuraNode::IO::BLOBOut* io)
: mPin(io)
{

}

void LEDStripOut::write(OSCMessage& msg) {
	if(!msg.isBlob(0)) {
		logger.log("not blob");
		return;
	}
//	logger.logf(SerialSysLog::DEBUG, "%d", msg.getBlobLength(0));
	mPin->writeValue(msg.getBlob(0), msg.getBlobLength(0));
}
